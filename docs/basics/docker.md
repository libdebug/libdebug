...
The container needs `/bin/sh` and, for an explicit environment dictionary, `/usr/bin/env`. Targets must use a CPU architecture supported by the host's ptrace backend.
...
With `env=None`, the target inherits the container environment. A dictionary replaces that environment; `{}` requests an empty environment. The requested dictionary is applied after the handshake shell, so shell variables such as `PWD` do not leak into it. Host debugging continues to inherit the host environment by default.
...
`runtime` accepts a Docker-compatible executable; omission tries Docker and Podman. Explicit `aslr=True` and `aslr=False` are rejected because this launch mechanism does not control container ASLR. `run(redirect_pipes=False)` is unsupported. Normal exit returns when the tracee exits, preserving unread output while a background thread reaps the runtime client. Detaching preserves the target and its output transport. Killing or terminating an active session cleans up the target and runtime client immediately. Startup waits for a complete PID handshake or EOF without a deadline; interrupting startup cancels the handshake and releases its resources.
...
```sh
python -m pytest --docker scripts/container_test.py
# Or use the unittest runner:
python run_suite.py docker
```

The suite builds a fixture image, starts actual containers, and traces compiled binaries. It requires Docker and host tracing permissions. Missing prerequisites and fixture failures are errors, not skips. The suite removes its own containers and image when it finishes. Default pytest collection deselects Docker tests, and the normal `fast` suite remains independent of Docker. The fixtures run as root, so the CI jobs run the checkout virtual environment's Python under `sudo` on the host to obtain tracing privileges. Docker socket access alone does not provide those privileges.
