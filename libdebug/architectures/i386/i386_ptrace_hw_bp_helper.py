...
I386_DBGREGS_CTRL_LOCAL = {"DR0": 1 << 0, "DR1": 1 << 2, "DR2": 1 << 4, "DR3": 1 << 6}
I386_DBGREGS_CTRL_COND = {"DR0": 16, "DR1": 20, "DR2": 24, "DR3": 28}
I386_DBGREGS_CTRL_COND_VAL = {"x": 0, "w": 1, "rw": 3}
I386_DBGREGS_CTRL_LEN = {"DR0": 18, "DR1": 22, "DR2": 26, "DR3": 30}
I386_DBGREGS_CTRL_LEN_VAL = {1: 0, 2: 1, 8: 2, 4: 3}

I386_DBREGS_COUNT = 4


class I386PtraceHardwareBreakpointManager(PtraceHardwareBreakpointManager):
    """A hardware breakpoint manager for the I386 architecture.
    ...
    def install_breakpoint(self, bp: Breakpoint):
        ...
        if self.breakpoint_count >= I386_DBREGS_COUNT:
        ...
        self.poke_user(self.thread.thread_id, I386_DBGREGS_OFF[register], bp.address)

        # Set the breakpoint control register
        ctrl = (
            I386_DBGREGS_CTRL_LOCAL[register]
            | (
                I386_DBGREGS_CTRL_COND_VAL[bp.condition]
                << I386_DBGREGS_CTRL_COND[register]
            )
            | (I386_DBGREGS_CTRL_LEN_VAL[bp.length] << I386_DBGREGS_CTRL_LEN[register])
        )

        # Read the current value of the register
        current_ctrl = self.peek_user(self.thread.thread_id, I386_DBGREGS_OFF["DR7"])

        # Clear condition and length fields for the current register
        current_ctrl &= ~(0x3 << I386_DBGREGS_CTRL_COND[register])
        current_ctrl &= ~(0x3 << I386_DBGREGS_CTRL_LEN[register])

        # Set the new value of the register
        current_ctrl |= ctrl

        # Write the new value of the register
        self.poke_user(self.thread.thread_id, I386_DBGREGS_OFF["DR7"], current_ctrl)
    ...
    def remove_breakpoint(self, bp: Breakpoint):
        ...
        self.poke_user(self.thread.thread_id, I386_DBGREGS_OFF[register], 0)

        # Read the current value of the control register
        current_ctrl = self.peek_user(self.thread.thread_id, I386_DBGREGS_OFF["DR7"])

        # Clear the breakpoint control register
        current_ctrl &= ~I386_DBGREGS_CTRL_LOCAL[register]
    ...
    def available_breakpoints(self) -> int:
        ...
        return I386_DBREGS_COUNT - self.breakpoint_count
