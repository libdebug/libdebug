...
class DebuggingContext:
    ...
    def attach(self: DebuggingContext, pid: int) -> None:
    ...
    def detach(self: DebuggingContext) -> None:
    ...
    def kill(self: DebuggingContext) -> None:
        """Kills the process."""
        try:
            self._ensure_process_stopped()
        except OSError:
    ...
    @background_alias(_background_invalid_call)
    def wait(self: DebuggingContext) -> None:
    ...
    def _craft_gdb_migration_command(self: DebuggingContext) -> list[str]:
    ...
    def _open_gdb_in_new_process(self: DebuggingContext) -> None:
    ...
    def _open_gdb_in_shell(self: DebuggingContext) -> None:
    ...
    def __threaded_cont(self: DebuggingContext) -> None:
        if self.argv:
            liblog.debugger(
    ...
    def __threaded_wait(self: DebuggingContext) -> None:
        if self.argv:
            liblog.debugger(
    ...
    def __threaded_migrate_to_gdb(self: DebuggingContext) -> None:
        self.debugging_interface.migrate_to_gdb()

    def __threaded_migrate_from_gdb(self: DebuggingContext) -> None:
        self.debugging_interface.migrate_from_gdb()

    def __threaded_peek_memory(
        self: DebuggingContext, address: int
    ) -> bytes | BaseException:
        try:
    ...
    def __threaded_poke_memory(
        self: DebuggingContext, address: int, data: bytes
    ) -> None:
    ...
    def _peek_memory(self: DebuggingContext, address: int) -> bytes:
    ...
    def _poke_memory(self: DebuggingContext, address: int, data: bytes) -> None:
