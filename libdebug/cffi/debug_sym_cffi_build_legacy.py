...
// Function to retrieve the debug file path from the gnu_debuglink and gnu_debugaltlink
// sections
void retrieve_debug_filename(Elf *elf)
{
    ...
    while ((section = elf_nextscn(elf, section)) != NULL)
    {
        ...
        char *name = elf_strptr(elf, ehdr.e_shstrndx, shdr.sh_name);
        if (name && (strcmp(name, ".gnu_debuglink") == 0 || strcmp(name, ".gnu_debugaltlink") == 0))
        {
...
// Function to read the symbol table, build ID, gnu_debuglink, and gnu_debugaltlink
SymbolInfo *read_elf_info(const char *elf_file_path, int debug_info_level)
{
    int fd;
    Elf *elf;
